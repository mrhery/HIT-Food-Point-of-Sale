...
if(isset($_GET["addon"], $_GET["item_id"], $_GET["type_name"])){
	...
	$qorder = mysqli_query($conn, "SELECT * FROM orders WHERE order_item = '{$item_name}' AND order_table = '{$addon}' AND order_ad = '{$order_ad}'");
	...
	if($sqorder == "0" || empty($sqorder)){
		...
		$iorder = mysqli_query($conn, "INSERT INTO orders(order_table, order_item, order_price, order_quantity, order_total, order_user, order_type, order_ad, order_adprint) VALUES('{$addon}', '{$item_name}', '{$item_price}', '{$quantity}', '{$total}', '{$order_user}', '{$type_name}', '{$order_ad}', '{$order_adprint}')");
		...
		$uorder = mysqli_query($conn, "UPDATE orders SET order_quantity = '{$new_order_quantity}', order_total = '{$newtotal}' WHERE order_id = '{$order_id}' AND order_ad = '{$order_ad}'");
