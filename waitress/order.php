...
if(isset($_GET["table_no"], $_GET["item_id"], $_GET["type_name"])){
	...
	$qorder = mysqli_query($conn, "SELECT * FROM orders WHERE order_item = '{$item_name}' AND order_table = '{$table_no}'");
	...
	if($sqorder == "0" || empty($sqorder)){
		...
		$iorder = mysqli_query($conn, "INSERT INTO orders(order_table, order_item, order_price, order_quantity, order_total, order_user, order_type) VALUES('{$table_no}', '{$item_name}', '{$item_price}', '{$quantity}', '{$total}', '{$order_user}', '{$type_name}')");
