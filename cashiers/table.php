...
<br /> 
    ...
    ?>
    <div id="rb_menu">
    	<a href="index.php">&larr; Go to Table's Interface</a> <a href="table.php?table_no=<?= $prev_table ?>">&larr; Prev Table</a> <a href="table.php?table_no=<?= $next_table ?>">Next Table &rarr;</a>
	</div>
    
    <div align="center" id="rb_menu" style="margin-top:100px;">
    	There is no Order for this Table yet.
        Add Order for this table:<br /><br /><br />
        <a href="add_order.php?table_no=<?= $table_no ?>">Add Order</a>
    </div>
    <?php 
		}else{
			$qbil_order = mysqli_query($conn, "SELECT * FROM bills WHERE bill_table = '{$table_no}' LIMIT 1");
    ...
    <br /><br />
    <div id="rb_menu">
    	<a href="paid.php?order_bill=<?= $rqbill_order["bill_no"] ?>&table_no=<?= $table_no ?>">Paid</a> 
        <?php
    		$qtype = mysqli_query($conn, "SELECT * FROM types");
			
			while($rqtype = mysqli_fetch_array($qtype)){
		?>
        <button onclick="popitup('print.php?type_name=<?= $rqtype["type_name"] ?>&table_no=<?= $table_no ?>&bill_no=<?= $bill_no ?>')">Print to <?= $rqtype["type_name"] ?></button>
		<?php
			}
        ?>
        <button onclick="popitup('print_bill.php?bill_no=<?= $bill_no ?>&table_no=<?= $table_no ?>')">Print Bill</button>
	</div>
	  ...
	  ?>
        ...
        <table width="80%" border="0" class="tbl" style="border:#F30 groove; border-top-right-radius:10px; border-top-left-radius:10px;">
            ...
            <?php
				...
				$q = mysqli_query($conn, "SELECT * FROM orders WHERE order_table = '{$table_no}'");
...
<div id="rb_menu" align="center">
    	<a href="added_order.php?table_no=<?= $table_no ?>">Edit Order</a>
	</div>
<?php
		}
?>
